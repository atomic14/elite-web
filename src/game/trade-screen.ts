...
/** What the Game should do next. The screens never change mode themselves. */
export type TradeOutcome =
  /** nothing to do */
  | 'stay'
  /** close the overlay */
  | 'close'
  /** close it, and the Game should also end the rock-hermit encounter */
  | 'leave-hermit';
...
export class TradeScreen {
  ...
  openMarket(ctx: TradeContext): void {
    this.marketSelected = 0;
    this.renderMarket(ctx);
  }

  renderMarket(ctx: TradeContext): void {
    renderMarket(
      ctx.atHermit ? { ...ctx.system, name: 'Rock Hermit' } : ctx.system,
      ctx.market, ctx.commander, this.marketSelected);
  }

  /** Click-to-select a market row. */
  selectMarketRow(index: number, ctx: TradeContext): void {
    this.marketSelected = index;
    this.renderMarket(ctx);
  }

  handleMarketInput(i: Input, ctx: TradeContext): TradeOutcome {
    const shift = i.held('ShiftLeft', 'ShiftRight');
    let changed = false;
    if (i.pressed('ArrowUp') || i.pressed('KeyW')) {
      this.marketSelected = (this.marketSelected + ctx.market.length - 1) % ctx.market.length;
      changed = true;
    }
    if (i.pressed('ArrowDown') || i.pressed('KeyS')) {
      this.marketSelected = (this.marketSelected + 1) % ctx.market.length;
      changed = true;
    }
    if (i.pressed('KeyB')) { this.buy(shift ? Infinity : 1, ctx); changed = true; }
    if (i.pressed('VirtBuyMax')) { this.buy(Infinity, ctx); changed = true; }
    if (i.pressed('KeyV')) { this.sell(shift ? Infinity : 1, ctx); changed = true; }
    if (i.pressed('VirtSellAll')) { this.sell(Infinity, ctx); changed = true; }
    if (i.pressed('Escape')) return ctx.atHermit ? 'leave-hermit' : 'close';
    if (changed) this.renderMarket(ctx);
    return 'stay';
  }

  /** Buy up to `want` units of the selected commodity (Infinity = fill up). */
  buy(want: number, ctx: TradeContext): void {
  ...
  sell(want: number, ctx: TradeContext): void {
    const idx = this.marketSelected;
    const m = ctx.market[idx];
    let sold = 0;
    let revenue = 0;
  ...
  openEquip(ctx: TradeContext): void {
    this.equipSelected = 0;
    renderEquip(ctx.system, ctx.commander, this.equipSelected, ctx.cheat);
  }

  /** Click-to-select an outfitter row. */
  selectEquipRow(index: number, ctx: TradeContext): void {
    this.equipSelected = index;
    renderEquip(ctx.system, ctx.commander, this.equipSelected, ctx.cheat);
  }

  handleEquipInput(i: Input, ctx: TradeContext): TradeOutcome {
    ...
    if (i.pressed('KeyB') || i.pressed('Enter')) {
      this.buyEquipment(rows[this.equipSelected].id, ctx);
      changed = true;
    }
    if (i.pressed('Escape')) return 'close';
    if (changed) renderEquip(ctx.system, ctx.commander, this.equipSelected, ctx.cheat);
    return 'stay';
  }

  buyEquipment(id: string, ctx: TradeContext): void {
